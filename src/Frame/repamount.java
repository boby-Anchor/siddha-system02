 ...
 * @author CHETHANA
 */
public class repamount extends javax.swing.JFrame {
 UniversalMethod u = new UniversalMethod();
 public static String d1;
  public static String d2;
    public static String t1;
    /**
     * Creates new form repamount
     */
    public repamount() {
        initComponents();
          systemLogger.initLogger(repamount.class).info("Open Repamount Frame");
   ...
   agent_la_home_img.setVisible(false);
    ...
    private void initComponents() {
        ...
        jTable_d_a.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Argent ID", "Argent Name", "Contact Number", "Amount", "Bank Name", "Baranch Name", "Account Number"
            }
        ) {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(repamount.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
...
//>>>>>>>>> cleareFields <<<<<<<<<<<      
    private void cleareFields() {
        DefaultTableModel defTabMod = (DefaultTableModel) jTable_d_a.getModel();
            ...
            jDateChooser_d1.setToolTipText("");
             jDateChooser_d2.setToolTipText("");
             d1="";
             d2="";
             la_netT.setText("");
             t1="";
    ...
    private void search(String d1 , String d2) {
       ...
       try{
           if(d1.equals("")||d2.equals("")){
            jDateChooser_d1.grabFocus();
        }else{
        ResultSet rs =DBClass.getData("select da.iddoctorrep,d.repname,d.reptell,d.repbankname,d.repbankbranchname,d.repbankaccountnumber "
                + ",sum(da.amount) as 'S' "
                + "from repamount da inner join doctorrep d on da.iddoctorrep = d.iddoctorrep"
                + " where da.date between '"+d1+"' and '"+d2+"' group by da.iddoctorrep ");
     
           while (rs.next()) {
                Vector v = new Vector();
                    v.add(rs.getString("da.iddoctorrep"));
                    v.add(rs.getString("d.repname"));
                    v.add(rs.getString("d.reptell"));
                    v.add(rs.getString("S"));
                     v.add(rs.getString("d.repbankname"));
                    v.add(rs.getString("d.repbankbranchname"));
                    v.add(rs.getString("d.repbankaccountnumber"));
                    defTabMod.addRow(v);
                }
           netToatl();
            }
           ...
           systemLogger.initLogger(repamount.class).error("search()  Error",e);
...
private void printInvoice() {
            ...
            try {
               // FileInputStream is = new FileInputStream("F:\\SADFInal\\01_1_siddha_System\\src\\Report\\RepAmount.jasper");
                  FileInputStream is = new FileInputStream(SaveFilePath.REPORT_File_path+"\\RepAmount.jasper");
                ...
                try {
                    ...
                    File file = new File(SaveFilePath.REPORT_BASE_URL + "\\DDRR4.txt");
                    if (!file.exists()) {
                        ...
                        fr.write("DB.map_Rep_Amo=" + pathPDF);
                        fr.close();
                        File pF = new File(pathPDF);
                        pF.mkdir();
                    }

                    InputStream input = new FileInputStream(SaveFilePath.REPORT_BASE_URL + "\\DDRR4.txt");
                    Properties prop = new Properties();
                    prop.load(input);
                    pathPDF = prop.getProperty("DB.map_Rep_Amo");
                    //  System.out.println(prop.getProperty("DB.mpa"));

                    JasperExportManager.exportReportToPdfFile(fileReport, pathPDF+ "\\"+"_"+d1+"_"+t1+"_"+d2+"_ Monthly Argent Amount.pdf");
                    ...
                    // System.out.println(pathPDF + "\\\\" + GenaratorPDFName(tx_refno_jLayeredPane_invoice.getText()));
                     ...
                     systemLogger.initLogger(repamount.class).error("printInvoice() IOException Error",e);
                    // e.printStackTrace();
                    JOptionPane.showMessageDialog(null, "PDF not save printInvoice!!! \n ", "Error Massage", JOptionPane.ERROR_MESSAGE);
                }
                JasperViewer.viewReport(fileReport);
              //   JasperPrintManager.printReport(fileReport, false);
                

            } catch (JRException e) {
                 systemLogger.initLogger(repamount.class).error("printInvoice() JRException Error",e);
                JOptionPane.showMessageDialog(null, "Repot Not Compleate , Please Check It printInvoice!!!", "Report Error", JOptionPane.ERROR_MESSAGE);
                 ...
                 systemLogger.initLogger(repamount.class).error("printInvoice() Exception Error",e);
                // e.printStackTrace();
...
//---------########---------*********************---------########---------*********************---------########---------\\ 
     ...
     */
//>>>>>>>>> Get Instance <<<<<<<<<<<  
    private static final repamount doctor = new repamount();

    public static repamount getCallPopup() {
        return doctor;
    }
//>>>>>>>>> Get Instance <<<<<<<<<<<         
...
private void netToatl() {
    ...
    for(int i=0;i<numnrow;i++){
        double val = Double.valueOf(jTable_d_a.getValueAt(i, 3).toString());
        nt += val;
    }
    la_netT.setText(""+nt);
